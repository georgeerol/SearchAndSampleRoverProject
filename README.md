...
#### Color Threshold
The mapping consisted of several steps, but the first one had to do with reading images from the rover camera. Training images were acquired from the simulation to calibrate and determine where the rover can drive. The simulation training environment consists of sand on the ground, which is very light in color and everything else, obstacles, in the environment are dark. Therefore Color Threshold was set to determine where the rover can drive by figuring out where the areas of lighter color are. The image processing was very simple; an RGB threshold was applied to the image to get it to recognize the different objects in the simulation environment.
Below are samples of color thresholds used to identify navigable terrain/obstacles/rock.
...
From the color threshold, the Rover was able to distinguish between obstacles, where to navigate and what’s a rock. However to know where and obstacle and where a rock is located, the perspective transform was used to each image that is provided from the rover camera to pinpoint the x and y position of each obstacle and rock. For the perspective transform to probably transform a source, and destination function was created and use so the image could be properly warped.
...
From the rover's camera, all navigable terrain pixel positions are extracted and then transform to “rover-centric” coordinates. The coordinate system allows us to describe the positions of objects in an environment with respect to the robot, in our case the rover’s camera. Meaning a coordinate frame where the rover's camera is at (x,y) = (0,0).
...
#### Pixel to World Map
The threshold images pixels values to rover centric coordinates are done for the terrain, rock, and obstacles.
The method `rover_coords` returns the x and y position for each of the white pixels from a threshold image then the function `pix_to_world` which converts the rover coordinates to the coordinates of the world, so showing what the rover's camera is filming.
...
*	`supportings_functions.py` contain three main methods:
...
The simulator ran on a resolution of 800x600 with the graphics set to Good.
...
* Add unit test for each method from the perception.py so one can see how each method works and not spend too much time on manual regression test.
